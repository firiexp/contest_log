...
void solve(int n, int l){
    ...
    for (int i = 0; i < l-1; ++i) {
        for (auto &&j : vv[i]) {
            if(vv[i+1].count(j.first) || j.second == 2) ret[i+1] = j.first;
        }
    }
    for (auto &&k : vv[0]) {
        if(ret[1] != k.first || k.second == 2) ret[0] = k.first;
    }
    for (auto &&k : vv[l-1]) {
        if(ret[l-1] != k.first || k.second == 2) ret[l] = k.first;
    }
    vector<int> ret2(ret);
