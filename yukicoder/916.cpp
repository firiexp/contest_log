...
int main() {
    ...
    Factorial f(1<<d);
    for (int i = 0; i < d; ++i) {
        p *= f[(1 << i)-(bsr(l) == i+1)-(bsr(r) == i+1)];
    }
    for (int i = 0; i < d; ++i) {
        if(bsr(l) <= i) break;
        else if(bsr(l) == i+1){
            if(bsr(r) != i+1 && bsr(r)-bsr(l) == k){
                ans += mint(1 << (bsr(r)-bsr(l)))*c*mint(500000004);
            }
        }else {
            if(bsr(l)+bsr(r)-2*(i+1) == k){
                ans += mint(1 << (bsr(r)-i-2))*mint(1 << (bsr(l)-i-2))*c;
            }
        }
        c *= 2;
    }
