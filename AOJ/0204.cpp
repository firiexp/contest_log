...
int main() {
    ...
    while(cin >> r >> n, n){
        vector<Point> v(n);
        vector<int> rs(n), vs(n), xs(n), ys(n);
        for (int i = 0; i < n; ++i) {
            cin >> xs[i] >> ys[i] >> rs[i] >> vs[i];
            v[i] = Point{real(xs[i]), real(ys[i])};
        }
        vector<int> defeated(n);
        for (int t = 1; ; ++t) {
            double dist = 1e9; int cur = -1;
            for (int i = 0; i < n; ++i) {
                if(defeated[i]) continue;
                if(xs[i]*xs[i]+ys[i]*ys[i] <= (ll)(t*vs[i]+r)*(t*vs[i]+r)){
                    defeated[i] = -1;
                } else if(abs(v[i])-t*vs[i] < dist){
                    assert(abs(v[i])-t*vs[i] >= 0);
                    dist = abs(v[i])-t*vs[i];
                    cur = i;
                }
            }
            if(!~cur) break;
            Segment S(polar(r, arg(v[cur])), v[cur]*10000);
            for (int i = 0; i < n; ++i) {
                if(defeated[i]) continue;
                Point vnow = polar(abs(v[i])-t*vs[i], arg(v[i]));
                Circle C(vnow, rs[i]);
                if(!crossPoint(C, S).empty()){
                    defeated[i] = 1;
                }
            }
        }
