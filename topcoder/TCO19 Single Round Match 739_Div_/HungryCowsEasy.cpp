...
class HungryCowsEasy {
    ...
    vector<int> findFood(vector<int> cowPositions, vector<int> barnPositions) {
        vector<int> v(cowPositions.size());
        for (int i = 0; i < cowPositions.size(); ++i) {
            ll dis = numeric_limits<ll>::max()/2, q = 0;
            for (int j = 0; j < barnPositions.size(); ++j) {
                if(abs(cowPositions[i]-barnPositions[j]) < dis){
                    dis = abs(cowPositions[i]-barnPositions[j]), q = j;
                }else if(abs(cowPositions[i]-barnPositions[j]) == dis){
                    if(barnPositions[j] < barnPositions[q]) q = j;
                }
            }
            v[i] = (int)q;
        }
