...
// CUT begin
	public static void main(String[] args){
		System.err.println("FindThePerfectTriangle (250 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            int perimeter = Integer.parseInt(Reader.nextLine());
            Reader.nextLine();
            ...
            if (doTest(area, perimeter, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1538345250;
        double PT = T / 60.0, TT = 75.0;
        System.err.println(String.format("Time   : %d minutes %d secs%nScore  : %.2f points", T / 60, T % 60, 250 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}

	static boolean doTest(int area, int perimeter, int[] __expected) {
		long startTime = System.currentTimeMillis();
		Throwable exception = null;
		FindThePerfectTriangle instance = new FindThePerfectTriangle();
		int[] __result = new int[0];
		try {
			__result = instance.constructTriangle(area, perimeter);
		}
	...
	static class Reader {
        private static final String dataFileName = "FindThePerfectTriangle.sample";
