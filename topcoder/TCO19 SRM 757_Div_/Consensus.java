...
// CUT begin
	public static void main(String[] args){
		System.err.println("Consensus (1000 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            int[] opinions = new int[Integer.parseInt(Reader.nextLine())];
            for (int i = 0; i < opinions.length; ++i)
                opinions[i] = Integer.parseInt(Reader.nextLine());
            Reader.nextLine();
            ...
            if (doTest(opinions, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1556588548;
	...
	static boolean doTest(int[] opinions, double __expected) {
		long startTime = System.currentTimeMillis();
		Throwable exception = null;
		Consensus instance = new Consensus();
		double __result = 0.0;
		try {
			__result = instance.expectedTime(opinions);
		}
	...
	static class Reader {
        private static final String dataFileName = "Consensus.sample";
