...
// CUT begin
	public static void main(String[] args){
		System.err.println("ThreePartSplit (250 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            if (doTest(a, d, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1544803176;
        double PT = T / 60.0, TT = 75.0;
        System.err.println(String.format("Time   : %d minutes %d secs%nScore  : %.2f points", T / 60, T % 60, 250 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}

	static boolean doTest(int a, int d, int[] __expected) {
		long startTime = System.currentTimeMillis();
		Throwable exception = null;
		ThreePartSplit instance = new ThreePartSplit();
		int[] __result = new int[0];
		try {
			__result = instance.split(a, d);
		}
	...
	static class Reader {
        private static final String dataFileName = "ThreePartSplit.sample";
