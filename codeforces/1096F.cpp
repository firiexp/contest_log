...
int main() {
    ...
    for (auto &&i : v) scanf("%d", &i), cnt[max(0, i)]++;
    vector<int> remain;
    for (int i = 1; i <= n; ++i) {
        if(!cnt[i]) remain.emplace_back(i);
    }
    Factorial f(n);
    ...
    for (int i = 0; i < n; ++i) {
        if(v[i] == -1){
            ans += mint(zeros)*invs[2];
            zeros++;
        }else {
            ans += mint(S.sum(n)-S.sum(v[i]));
            S.add(v[i], 1);
            if(!remain.empty()){
                int x = remain.size() - (lower_bound(remain.begin(),remain.end(), v[i])-remain.begin());
                ans += mint(zeros)*mint(x)*invs[remain.size()];
            }
        }
    }
    zeros = 0;
    for (int i = n-1; i >= 0; --i) {
        if(v[i] != -1){
            if(!remain.empty()){
                int x = lower_bound(remain.begin(),remain.end(), v[i])-remain.begin();
                ans += mint(zeros)*mint(x)*invs[remain.size()];
            }
        }else zeros++;
    }
